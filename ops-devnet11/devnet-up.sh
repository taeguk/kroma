...
if [ ! -f "$DEVNET/done" ]; then
  ...
  cat "$CONTRACTS/deploy-config/$DEVNET_NAME.json" | jq -r ".l1GenesisBlockTimestamp = \"$TIMESTAMP\"" >/tmp/devnet-deploy-config.json
  ...
  (
    ...
    go run cmd/main.go genesis l2 \
      ...
      --deployment-dir "$CONTRACTS/deployments/$DEVNET_NAME" \
...
L2OO_ADDRESS=$(cat "$CONTRACTS/deployments/$DEVNET_NAME/L2OutputOracleProxy.json" | jq .address)
COLOSSEUM_ADDRESS=$(cat "$CONTRACTS/deployments/$DEVNET_NAME/ColosseumProxy.json" | jq .address)
VALPOOL_ADDRESS=$(cat "$CONTRACTS/deployments/$DEVNET_NAME/ValidatorPoolProxy.json" | jq .address)
SECURITYCOUNCIL_ADDRESS=$(cat "$CONTRACTS/deployments/$DEVNET_NAME/SecurityCouncilProxy.json" | jq .address)
