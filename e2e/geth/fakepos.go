...
import (
	...
	keth "github.com/kroma-network/kroma/components/node/eth"
	...
	f.sub = event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case now := <-t.Ch():
				...
				isCancun := f.eth.BlockChain().Config().IsCancun(newBlockTime)
				if isCancun {
				...
				for _, commitment := range envelope.BlobsBundle.Commitments {
					...
					blobHashes = append(blobHashes, keth.KZGToVersionedHash(*(*[48]byte)(commitment)))
				}
