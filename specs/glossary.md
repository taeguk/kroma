...
[syncer]: glossary.md#syncer

A syncer is an entity (individual or organization) that runs a [rollup node][rollup-node] in syncer mode.
...
It also lets the syncer verify the work of the [sequencer], by re-deriving [output roots][l2-output] and comparing
them against those submitted by the sequencer. In case of a mismatch, the syncer can perform a [zk-fault
proof][zk-fault-proof].
